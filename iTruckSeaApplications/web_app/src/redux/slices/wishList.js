...
export const wishListSlice = createSlice({
  ...
  reducers: {
    ...
    removeWishListFood: (state = initialState, action) => {
      let tempWishList = state.wishLists.food?.filter((item) => item.id !== action.payload);
      return {
        wishLists: {
          ...state.wishLists,
          food: [...tempWishList],
    ...
    removeWishListRes: (state = initialState, action) => {
      let tempWishList = state.wishLists.restaurant?.filter((item) => item.id !== action.payload);
      return {
        wishLists: {
          ...state.wishLists,
