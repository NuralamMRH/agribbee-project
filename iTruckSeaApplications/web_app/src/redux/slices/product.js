...
const slice = createSlice({
  ...
  reducers: {
    ...
    getCart(state, action) {
      ...
      state.checkout.payment_method = state.checkout.payment_method;
      state.checkout.sourcer = state.checkout.sourcer;
      state.checkout.sourcer_id = state.checkout.sourcer_id;
      state.checkout.sourcer_name = state.checkout.sourcer_name;
    },
    ...
    setPaymentMethod(state, action) {
      state.checkout.payment_method = action.payload;
    },
    setSourcerCheckout(state, action) {
      state.checkout.sourcer = action.payload;
    },
    setSourcerCheckoutId(state, action) {
      state.checkout.sourcer_id = action.payload;
    },

    setSourcerName(state, action) {
      state.checkout.sourcer_name = action.payload;
    },
...
export function getProducts() {
  return async (dispatch) => {
    ...
    try {
      const response = await MainApix.get('/api/v1/me/products');
...
export function getProduct(name) {
  return async (dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await MainApix.get('/api/v1/products/product', {
        params: { name },
      });
