...
import { setOrderInvoice } from 'src/redux/slices/storedData';
import { useSnackbar } from 'notistack';
import MainApi from 'src/api/MainApi';
import { dispatch } from 'src/redux/store';
import { useSelector } from 'react-redux';
import { setPaymentMethod, setSourcerCheckoutId, setSourcerName } from 'src/redux/slices/product';
...
export default function CheckoutPayment({
  ...
  const onSubmit = async () => {
    try {
      ...
      dispatch(setPaymentMethod(values.payment));

      const postOrderRequest = await MainApi.post(`/api/v1/submit-order-request`, checkout);
      console.log('onSubmit', postOrderRequest.data);

      if (postOrderRequest.data.success) {
        dispatch(setOrderInvoice(postOrderRequest.data.order));
