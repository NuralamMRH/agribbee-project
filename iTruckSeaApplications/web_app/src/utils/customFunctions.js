...
export const getNumberWithConvertedDecimalPoint = (amount, digitAfterDecimalPoint) => {
  let newNumber = ((amount * 100) / 100).toFixed(Number.parseInt(digitAfterDecimalPoint));
  return newNumber;
};
...
export const getSubTotalPrice = (cartList) => {
  ...
  let ad = cartList.reduce(
    (total, product) =>
      (product.variations.length > 0 ? getItemTotalWithoutDiscount(product) : product.price) *
        product.quantity +
...
export const getFinalTotalPrice = (items, couponDiscount, taxAmount, restaurantData) => {
  let totalPrice = 0;
  if (items?.length > 0) {
    items.map((item) => {
      totalPrice +=
        item.price * item.quantity - getProductDiscount(items, restaurantData) + taxAmount;
    });
    if (couponDiscount && couponDiscount?.discount)
      return totalPrice - getCouponDiscount(couponDiscount, restaurantData?.data, items);
    return totalPrice;
  }
  return totalPrice;
};
...
export const getTaxableTotalPrice = (items, couponDiscount, restaurantData, referDiscount) => {
  ...
  let total =
    ...
    (couponDiscount ? getCouponDiscount(couponDiscount, restaurantData, items) : 0) -
...
export const getProductDiscount = (items, restaurantData) => {
  if (restaurantData?.data?.discount) {
    ...
    let combinedEndDateTime = moment(`${endDate} ${endTime}`, 'YYYY/MM/DD HH:mm:ss').format();
    ...
    if (combinedEndDateTime > currentDateTime) {
      ...
      let restaurentMinimumPurchase = restaurantData?.data?.discount?.min_purchase;
      let restaurentMaxDiscount = restaurantData?.data?.discount?.max_discount;
    ...
    let totalDiscount = items?.reduce((total, product) => {
      ...
      return (
        total + (handleProductValueWithOutDiscount(product) - discountAmount) * product.quantity
      );
    }, 0);
    return totalDiscount;
  }
...
const handleGlobalDeliveryFee = (global, totalOrderAmount, orderType, deliveryFee, extraCharge) => {
  if (
  ...
  if (Number.parseInt(restaurantData?.data?.self_delivery_system) === 1) {
    if (
      ...
      (restaurantData?.data?.free_delivery_distance_status &&
        convertedDistance < restaurantData?.data?.free_delivery_distance_value)
    ) {
      return 0;
    } else {
      deliveryFee = convertedDistance * restaurantData?.data?.per_km_shipping_charge || 0;
      if (
        ...
        if (deliveryFee < restaurantData?.data?.minimum_shipping_charge) {
    ...
    if (zoneData?.length > 0) {
      const restaurantChargeInfo = zoneData?.find(
        (item) => Number.parseInt(item.id) === Number.parseInt(restaurantData?.data?.zone_id)
      );
      ...
      if (
        ...
        if (
          ...
          deliveryFee = convertedDistance * (restaurantChargeInfo?.per_km_shipping_charge || 0);
          if (
            deliveryFee >= restaurantChargeInfo?.minimum_shipping_charge &&
            deliveryFee + extraCharge <= restaurantChargeInfo?.maximum_shipping_charge
          ) {
            return (
...
export const getConvertDiscount = (dis, disType, price, restaurantDiscount, quantity) => {
    ...
    if (purchasedAmount >= couponDiscount.min_purchase) {
      switch (couponDiscount.coupon_type) {
          ...
          if (Number.parseInt(zoneid[0]) === couponDiscount.zoneId) {
            if (couponDiscount && couponDiscount.discount_type === 'amount') {
              ...
              let percentageWiseDis =
                (purchasedAmount - getProductDiscount(cartList, restaurantData)) *
        ...
        case 'restaurant_wise':
          ...
          if (Number.parseInt(restaurantId[0]) === restaurantData?.data?.id) {
            if (couponDiscount && couponDiscount.discount_type === 'amount') {
              if (couponDiscount.max_discount === 0) {
                return couponDiscount.discount;
              } else {
...
export const isAvailable = (start, end) => {
  const startTime = moment(start); // Directly parse ISO date-time format
  const endTime = moment(end); // Directly parse ISO date-time format

  // Ensure you are working with the same timezone (for example, Asia/Ho_Chi_Minh)
...
export const getCalculatedTotal = (
  ...
  if (couponDiscount) {
    ...
    return (
      ...
      (global?.tax_included !== 1 &&
        truncate(
          getTaxableTotalPrice(cartList, couponDiscount, restaurantData, referDiscount)?.toString(),
          global?.digit_after_decimal_point
        )) -
      0 +
      truncate(
...
export const getPriceByPriorityAndDiscount = (product) => {
  if (product.restaurant_discount === 0) {
    return getConvertDiscount(product.discount, product.discount_type, product.price);
  } else {
    let price = product.price - (product.price * product.restaurant_discount) / 100;
...
export const getIndexFromArrayByComparision = (arrayOfObjects, object) => {
  ...
  return arrayOfObjects.findIndex(
    (item) => _.isEqual(item.variations, object.variations) && item.id === object.id
  );
};
export const handleTotalAmountWithAddons = (mainTotalAmount, selectedAddOns) => {
...
export const maxCodAmount = (restaurantData, global, zoneData) => {
  if (zoneData?.data?.zone_data?.length > 0) {
    ...
    if (resInfor?.max_cod_order_amount !== null && resInfor?.max_cod_order_amount !== 0) {
...
export const languageValue = (language) => {
  ...
  let selectedLanguage = allLangs?.find((item) => {
...
export const DistanceCalculate = ({ distance }) => {
  ...
  const distanceValue = (distance / 1000).toFixed(global?.digit_after_decimal_point);

  const getDistance = () => {
    if (Number.parseInt(distanceValue) > 1000) {
      return t('1k+ km');
    } else {
      return `${(distance / 1000).toFixed(global?.digit_after_decimal_point)}km `;
    }
  };
