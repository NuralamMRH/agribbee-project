...
export const AuctionCountdown = ({
  ...
  if (!hasStarted) {
    return (
      <>
        ...
        {`${timeLeftToStart.days?.toString().padStart(2, '0')}d ${timeLeftToStart.hours
          ?.toString()
          .padStart(2, '0')}h ${
          isShowMinSec
            ? `${timeLeftToStart.minutes?.toString().padStart(2, '0')}m ${timeLeftToStart.seconds
                ?.toString()
                .padStart(2, '0')}s`
            : ''
        }`}
  ...
  if (hasStarted && !hasEnded) {
    return (
      <>
        {isHelpText && 'Ends in '}
        {`${timeLeftToEnd.days?.toString().padStart(2, '0')}d ${timeLeftToEnd.hours
          ?.toString()
          .padStart(2, '0')}h${
          isShowMinSec
            ? `:${timeLeftToEnd.minutes?.toString().padStart(2, '0')}:${timeLeftToEnd.seconds
                ?.toString()
                .padStart(2, '0')}`
            : ''
        }`}
  ...
  return <>{`${isEndText ? isEndText : 'Ended'}`}</>;
};
