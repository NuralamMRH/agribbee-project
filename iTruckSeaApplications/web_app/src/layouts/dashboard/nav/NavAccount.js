...
const StyledRoot = styled('div')(({ theme, borderRadius }) => ({
  display: 'flex',
  alignItems: 'center',
  padding: theme.spacing(2, 2.5),
  borderRadius:borderRadius ? borderRadius : Number(theme.shape.borderRadius) * 1.5,
...
export default function NavAccount({ borderRadius }) {
    ...
    <Link component={NextLink} href={PATH_DASHBOARD.user.account} underline="none" color="inherit">
      <StyledRoot borderRadius={borderRadius}>
        <CustomAvatar
          ...
          name={user?.f_name ? `${user?.f_name} ${user?.l_name}` : user?.email}
        />
