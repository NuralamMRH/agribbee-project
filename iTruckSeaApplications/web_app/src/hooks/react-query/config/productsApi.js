...
export const ProductApis = {
  ...
  searchlatestFood: ({ restaurant_id, searchKey, type, offset, page_limit }) => {
    return MainApi.get(
...
export const ProductsApi = {
  reviewed: () => MainApi.get('/api/v1/products/most-reviewed'),
  popularFood: () => MainApi.get('/api/v1/products/popular'),
  latestFood: ({ restaurant_id, category_id, type, pageOffset, pageLimit }) => {
    return MainApi.get(
  ...
  productSearch: (search_type, value, offset, page_limit, filterData) => {
    ...
    if (value !== '') {
      return MainApi.get(
        ...
        }&discounted=${filterData?.filterBy?.discounted ? 1 : 0}&sort_by=${filterData?.sortBy}`
      );
    }
  },

  addFavorite: (product_id) => {
    return MainApi.post(`/api/v1/customer/wish-list/add?food_id=${product_id}`);
  },
