...
export const AuctionsApi = {
  filteredAuctions: ({ filter }) => {
    filter
      ? MainApix.get(`/api/v1/auctions/?${filter}`)
      : MainApix.get(`/api/v1/auctions`)
  },
  ...
  auctions: (auction_type, limit, offset) =>
    MainApix.get(
      `/api/v1/auctions?auction_type=${auction_type}&limit=${limit}&offset=${offset}`
    ),
  ...
  regionAuctions: (
    ...
    return MainApix.get(
      `/api/v1/auctions?auction_type=${auction_type}${category_id && `&category_id=${category_id}`}${region_id ? `&warehouse_region=${region_id}` : ""}${keyword ? `&keyword=${keyword}` : ""}${shipping_end_date ? `&shipping_end_date[gte]=${shipping_end_date}` : ""}&limit=${limit}&offset=${offset}`
    )
  },
  ...
  addFavoriteAuction: (auction_id) => {
    return MainApix.post(
      `/api/v1/customer/wish-list/add?auction_id=${auction_id}`
    )
  },
...
export const ProductsApi = {
  ...
  addFavorite: (product_id) => {
    return MainApix.post(
      `/api/v1/customer/wish-list/add?product_id=${product_id}`
    )
  },
  addFavoriteAuction: (auction_id) => {
    return MainApix.post(
