...
const SignInPage = ({
  ...
  const [signBy, setSignBy] = useState("email");
  ...
  //Login By Email
  const loginByEmailFormik = useFormik({
    initialValues: {
    ...
    validationSchema: Yup.object({
      email: Yup.string()
        .min(4, t("Email need to type properly."))
        .required(t("Password is required")),
      password: Yup.string()
        .min(6, t("Password is too short - should be 6 chars minimum."))
        .required(t("Password is required")),
  ...
  const handleTokenAfterSignIn = async (response) => {
    if (response?.data) {
      // Set tokens for frontend application
      localStorage.setItem("token", response?.data?.token);
      localStorage.setItem("accessToken", response?.data?.token);
      localStorage.setItem("role", response?.data?.role || "user");
      localStorage.setItem("agribbeeUserToken", response?.data?.token);
      ...
      const dashboardUrl = process.env.NEXT_PUBLIC_DASHBOARD || "http://localhost:3033";
    ...
    loginMutation(newValues, {
      onSuccess: async (response) => {
        if (global?.customer_verification) {
          if (Number.parseInt(response?.data?.is_phone_verified) === 1) {
  ...
  const {
    mutate: loginEmailMutation,
    isEmailLoginLoading,
    emailError,
  } = useMutation("email-sign-in", AuthApi.signInEmail);

  const formSubmitEmailHandler = (values) => {
    const newValues = { ...values, guest_id: guestId };
    loginEmailMutation(newValues, {
      onSuccess: async (response) => {
        console.log("Login Response: ", response);
        if (global?.customer_verification) {
          if (Number.parseInt(response?.data?.is_email_verified) === 1) {
            handleTokenAfterSignIn(response);
            handleClose?.();
          } else {
  ...
  return (
    <Stack>
      <RTL direction={languageDirection}>
        <CustomStackFullWidth alignItems="center" spacing={{ xs: 0.5, md: 2 }}>
          <CustomStackFullWidth alignItems="center" spacing={{ xs: 1, md: 0 }}>
            <CustomImageContainer
              ...
              alt="Logo"
            />
            <CustomTypography sx={{ fontWeight: "bold", fontSize: "22px" }}>
              {t("Sign In")}
            </CustomTypography>
          </CustomStackFullWidth>
          ...
          {signBy === "email" ? (
            <CustomStackFullWidth
              ...
              spacing={{ xs: 1, md: 2 }}
            >
              <form onSubmit={loginByEmailFormik.handleSubmit} noValidate>
                <CustomStackFullWidth
                  alignItems="center"
                  spacing={{ xs: 2, md: 4 }}
                >
                  <FormControl variant="outlined" fullWidth>
                    ...
                    <CustomSigninOutLine
                      ...
                      value={loginByEmailFormik.values.email}
                      onChange={loginByEmailFormik.handleChange}
                      error={
                        loginByEmailFormik.touched.email &&
                        Boolean(loginByEmailFormik.errors.email)
                      }
                      helperText={
                        loginByEmailFormik.touched.email &&
                        loginByEmailFormik.errors.email
                      }
                      touched={loginByEmailFormik.touched.email}
                      startAdornment={
                        <InputAdornment
                          position="start"
                          sx={{ marginInlineEnd: "0px !important" }}
                        >
                          <IconButton
                  ...
                  <FormControl variant="outlined" fullWidth>
                    ...
                    <CustomSigninOutLine
                      ...
                      value={loginByEmailFormik.values.password}
                      onChange={loginByEmailFormik.handleChange}
                      error={
                        loginByEmailFormik.touched.password &&
                        Boolean(loginByEmailFormik.errors.password)
                      }
                      helperText={
                        loginByEmailFormik.touched.password &&
                        loginByEmailFormik.errors.password
                      }
                      touched={loginByEmailFormik.touched.password}
                      endAdornment={
          ...
          >
            <CustomTypography
              onClick={() =>
                signBy === "email" ? setSignBy("phone") : setSignBy("email")
              }
              fontSize="14px"
              fontWeight="400"
              sx={{ cursor: "pointer" }}
            >
              {t(`Login using ${signBy === "email" ? "Phone" : "Email"}`)}
            </CustomTypography>
          </CustomStackFullWidth>

          {global?.social_login.length > 0 &&
            global?.social_login?.some((item) => item.status === true) && (
              <CustomStackFullWidth
                alignItems="center"
                justifyContent="center"
                spacing={1}
