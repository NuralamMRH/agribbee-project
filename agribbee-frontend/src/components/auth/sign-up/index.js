...
import { Grid, Switch, alpha, styled } from "@mui/material";
...
import { PackageApi } from "@/hooks/react-query/config/packageApi";
import { useGetPackages } from "@/hooks/react-query/packages/useGetPackages";
...
const SignUpPage = ({
  ...
  const [isCheckedPackage, setIsCheckedPackage] = useState();
  ...
  const { mutate, isLoading } = useMutation("sign-up", AuthApi.signUp);
  ...
  const {
    ...
    ["packages"],
    () => PackageApi.packages(), // Pass offset and limit as well
    {
      enabled: false,
      staleTime: 1000 * 60 * 8,
  ...
  const handleCheckMembership = (selectedId) => {
    // Update the Formik state to only have the selected package ID
    signUpFormik.setFieldValue("membership", selectedId);
  };
  ...
  return (
    <Stack>
      <RTL direction={languageDirection}>
        <CustomStackFullWidth alignItems="center" spacing={{ xs: 0.5, md: 3 }}>
            ...
            <Stack>
              <Grid container spacing={3}>
                <Grid item xs={12} md={12} textAlign={"center"}>
                  {packages?.data?.packages.map((pak) => (
                    <FormControlLabel
                      control={
                        <IOSSwitch
                          value={pak}
                          onChange={() => handleCheckMembership(pak?._id)}
                          sx={{ m: 1 }}
                          checked={signUpFormik.values.membership === pak?._id} // Ensure only one is checked
                        />
                      }
                      label={pak?.name}
                    />
                ...
                <Grid item xs={12} md={12}>
                  <CustomSignUpTextField
                    ...
                    error={
                      ...
                      Boolean(signUpFormik.errors.company_name)
                    }
                    helperText={
                      signUpFormik.touched.company_name &&
                      signUpFormik.errors.company_name
                    }
                    touched={signUpFormik.touched.company_name}
                    InputProps={{
                      startAdornment: (
                        <InputAdornment position="start">
                          <Work
                  ...
                  <FormControl variant="outlined" fullWidth>
                    ...
                    <CustomSigninOutLine
                      ...
                      error={
                        ...
                        Boolean(signUpFormik.errors.confirm_password)
                      }
                      helperText={
                        signUpFormik.touched.confirm_password &&
              ...
              <CustomStackFullWidth>
                ...
                {signUpFormik.touched.tandc && signUpFormik.errors.tandc && (
                  <CustomTypography
                    variant="caption"
                    sx={{
                      fontWeight: "inherit",
