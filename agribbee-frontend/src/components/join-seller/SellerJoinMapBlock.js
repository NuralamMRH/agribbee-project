...
import { FacebookCircularProgress } from "../landingpage/HeroLocationForm";
import { CustomTypographyGray } from "@/styled-components/CustomTypographies.style";
import { PrimaryButton } from "../landingpage/link-section/Linksection.style";
import LocationEnableCheck from "../landingpage/LocationEnableCheck";
...
const SellerJoinMapBlock = ({
  ...
  useEffect(() => {
    if (zoneData) {
      ...
      formData.setFieldValue("zip", zoneData?.data?.zone_id);
  ...
  useEffect(() => {
    formData.setFieldValue("address", currentLocationValue?.description);
  ...
  const handleLocationSelection = (value) => {
    ...
    formData.setFieldValue("address", value?.description);
  };
...
export default SellerJoinMapBlock;
