...
import { ProductsApi } from "@/hooks/react-query/config/auctionsApi";
import { useGetProductDetails } from "@/hooks/react-query/product/useGetProductDetails";
...
const ProductDetailModal = ({
  ...
  const {
    ...
    isRefetching,
  } = useGetProductDetails({ id: product?._id, campaign }, itemSuccess);
  ...
  const handleAddUpdate = () => {
    if (productUpdate) {
      ...
      const itemObject = {
        ...
        guest_id: getGuestId(),
        model: product?.available_date_starts ? "ItemCampaign" : "Product",
        add_on_ids:
          add_on?.length > 0
            ? add_on?.map((add) => {
                return add.id;
  ...
  const {
    ...
    error,
    data,
  } = useMutation("add-favorite", () => ProductsApi.addFavorite(product._id), {
    onSuccess: (response) => {
      if (response?.data) {
  ...
  const onSuccessHandlerForDelete = (res) => {
    dispatch(removeWishListFood(product._id));
      ...
      modalData[0]?.variations?.forEach((variation, index) => {
        if (variation?.type === "multi") {
          ...
          const singleVariation = modalData[0]?.variations?.filter(
            (item) => item?.type === "single" && item?.required === "on"
          );
      ...
      >
        <FoodDetailModalStyle sx={{ bgcolor: "background.paper" }}>
          {!itemIsLoading && modalData[0] ? (
            <>
              {isLocation ? (
                ...
                <CustomStackFullWidth>
                  ...
                  <CustomStackFullWidth sx={{ padding: "20px" }} spacing={2}>
                    <SimpleBar
                      style={{
                        maxHeight: "35vh",
                        paddingInline: "10px",
                    ...
                    >
                      <CustomStackFullWidth spacing={0.5}>
                        ...
                        >
                          ...
                          <VagSvg
                            color={
                              Number(modalData[0]?.veg) === 0
                                ? theme.palette.nonVeg
                                : theme.palette.success.light
                            }
                          />
                          {modalData[0]?.halal_tag_status === 1 &&
                            modalData[0]?.is_halal === 1 && (
                          ...
                          {quantity >= modalData[0]?.item_stock &&
                            modalData[0]?.stock_type !== "unlimited" && (
                              <Typography
                                fontSize="12px"
                                color={
                    ...
                    <Grid container direction="row">
                      <Grid item md={7} sm={12} xs={12} alignSelf="center">
                        <TotalAmountVisibility
                          ...
                          digitAfterDecimalPoint={digitAfterDecimalPoint}
                          t={t}
                          productDiscount={modalData[0]?.discount}
                          productDiscountType={modalData[0]?.discount_type}
            ...
            !productUpdate && (
              <CustomStackFullWidth sx={{ padding: "10px" }} spacing={1}>
                <Skeleton variant="rectangular" witdh="100%" height="200px" />
