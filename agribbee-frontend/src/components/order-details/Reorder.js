...
const Reorder = ({ orderData, orderZoneId }) => {
  ...
  const { mutate: reorderAddToCartMutate, isLoading: addToCartLoading } =
    useReorderAddToCart();
  const orderItemId = () => {
    let currentId = [];
  ...
  const getNewValues = (
    ...
    return variationValues?.map((item, index) => {
      let isExist = sVariationValues?.find(
        (sItem) => sItem?.label === item?.label
      );
      if (isExist) {
        return {
          ...item,
  ...
  const reorderAddToCart = () => {
    ...
    const isZoneExist =
      orderAbleZoneIds?.length > 0 &&
      orderAbleZoneIds.find((item) => item === orderZoneId);
    if (isZoneExist) {
