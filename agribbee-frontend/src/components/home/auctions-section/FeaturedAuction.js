...
const FeaturedAuction = ({}) => {
  ...
  const {
    data: featuredAuctionsData,
    refetch: refetchFeaturedAuctions,
    isLoading: featuredIsLoading,
  } = useQuery(
    ["live_auction"],
    () => AuctionsApi.auctions(auction_type, limit), // Pass offset and limit as well
    {
      enabled: false,
      staleTime: 1000 * 60 * 8,
  ...
  return (
    ...
    >
      <CustomContainer>
        {featuredAuctionsData?.data?.auctions?.length > 0 && (
          <Stack
            direction="row"
            paddingTop="20px"
            paddingBottom="20px"
        ...
        <CustomStackFullWidth>
          {!featuredIsLoading ? (
            <CustomStackFullWidth>
              ...
              >
                <Slider {...settings}>
                  {featuredAuctionsData?.data?.auctions?.map((auction) => {
                    return (
                      <AuctionCard
                        key={auction?.id}
                        auction={auction}
...
export default memo(FeaturedAuction);
