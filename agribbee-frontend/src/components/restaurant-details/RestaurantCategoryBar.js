...
const RestaurantCategoryBar = (props) => {
  ...
  useEffect(() => {
    if (selectedId && refs.current[selectedId]) {
      ...
      const offset =
        buttonLeft -
        scrollerLeft +
        (selectedButton.offsetWidth - scrollerRef.current.offsetWidth) / 2;
  ...
  const handleFilterData = (event, id) => {
    ...
    const filteredData = {
      veg: activeFilters.find((filter) => filter.value === "veg") !== undefined,
      nonVeg:
        activeFilters.find((filter) => filter.value === "nonVeg") !== undefined,
      currentlyAvailable:
        activeFilters.find(
                ...
                {data?.map((item, index) => {
                  return (
                    <CategoryButton
                      ...
                      active={isActiveCategoryBar(item, index)}
                    >
                      <Typography
                        fontSize={{
                          xs: "12px",
