...
import ProductDetailModal from "../productDetail-modal/ProductDetailModal";
...
const ProductCard = ({
  ...
  const handleProductDetailModal = (e) => {
    e.stopPropagation();
    setOpenModal(true);
  };
  ...
  const {
    ...
    "add-favourite",
    () => ProductsApi.addFavorite(product?._id),
    {
      onSuccess: (response) => {
        if (response?.data) {
      ...
      onError: (error) => {
        ...
        alert("Toasted from ProductCard ", error.response.data.message);
      },
    }
  );
  ...
  const isInList = (id) => {
    return !!wishLists?.food?.find((wishFood) => wishFood?._id === id);
  };
  const isInCart = cartList?.find((things) => things?._id === product?._id);
  ...
  const addToCartHandler = () => {
    ...
    const itemObject = {
      guest_id: getGuestId(),
      model: modalData[0]?.available_date_starts ? "ItemCampaign" : "Product",
      add_on_ids: [],
      add_on_qtys: [],
  ...
  const addToCart = (e) => {
    ...
    // if (product?.stock === 0) {
    //   e.stopPropagation();
    //   CustomToaster("error", t("Out Of Stock"), product?._id);
    // } else {
    // }
  };
  const getQuantity = (id) => {
    const product = cartList.find((cartItem) => cartItem._id === id);
