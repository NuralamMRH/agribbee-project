...
const ProfilePage = () => {
  ...
  const { mutate, isLoading: deleteUserIsLoading } = useUserDelete(
    ...
    <>
      ...
      {data ? (
        <CustomStackFullWidth gap="15px">
          {((!isSmall && isEditProfile === false) ||
            (isSmall && isEditProfile === false)) && (
            <SliderCustom languageDirection={languageDirection} gap="0">
              <Slider {...settings}>
                <ProfileStatistics
          ...
          <Stack
            gap={isEditProfile ? 0 : "15px"}
            paddingInline={{ xs: "0", sm: "2px 10px" }}
          >
            <CustomPaperBigCard
