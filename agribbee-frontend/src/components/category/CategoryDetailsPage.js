...
import FoodOrRestaurant from "../products-page/AuctionTabContainer";
...
import { setGlobalTabType } from "@/redux/slices/searchFilter";
...
const CategoryDetailsPage = ({
  ...
  const { globalTabType } = useSelector((state) => state.searchFilterStore);
  ...
  const getFoodOrRestaurant = (value) => {
    setGlobalTabType(value);
  };
  useEffect(() => {
    if (globalTabType === "restaurants") {
      setOffset(1);
    }
  }, [globalTabType]);
  useEffect(() => {
      ...
      <Grid container spacing={{ xs: 1, sm: 3, md: 2 }}>
        ...
        <Grid item xs={12} sm={12} md={12} align="left" mt="0rem">
          <CustomStackFullWidth
            direction="row"
            justifyContent={
              catetoryMenus?.length > 0 ? "space-between" : "flex-end"
            }
            alignItems="center"
          >
        ...
        <Grid
          ...
          md={12}
          container
          spacing={{ xs: 1, md: globalTabType === "products" ? 2 : 4 }}
        >
