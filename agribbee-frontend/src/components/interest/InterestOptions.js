...
const InterestOptions = (props) => {
  ...
  const handleItemClick = (item) => {
    ...
    const isItemAlreadyExist = selectedId.find((id) => id === item._id);
    if (isItemAlreadyExist) {
      const newArray = selectedId.filter((id) => id !== item._id);
      setSelectedId(newArray);
    } else {
      setSelectedId((prevState) => [...prevState, item._id]);
    }
  };
  ...
  return (
    ...
    >
      ...
      <CustomTypographyGray variant="h4" nodefaultfont="true">
        {t("Get personalized auctions and products notification.")}
            ...
            return (
              ...
              >
                <CustomPaperBigCard
                  padding=".5rem"
                  sx={{
                    border: handleBorder(item._id) && "2px solid",
                    borderColor: handleBorder(item._id) && "primary.main",
                  }}
                >
