...
export const getFilterChoices = (filterData, data, globalTabType) => {
  let productsList =
  ...
  if (productsList && productsList.length > 0) {
    if (filterData.rating !== "") {
      productsList = productsList.filter(
        (product) => Number(product.avg_rating) >= Number(filterData.rating)
      );
    }
    ...
    if (filterData?.filterBy?.new) {
      productsList = productsList
        ...
        .map((momentObject) => {
          return {
            ...momentObject,
            created_at: momentObject.created_at?.format("YYYY/MM/DD HH:mm A"),
          };
        })
        .slice(0, 16);
    }
    ...
    if (globalTabType === "products") {
      if (filterData.price !== "") {
        productsList = productsList.filter(
          (product) =>
            Math.floor(product.price) >= Math.floor(filterData.price[0]) &&
            Math.floor(product.price) <= Math.floor(filterData.price[1])
        );
      }
    }
  }
