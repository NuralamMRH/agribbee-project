...
const SecondNavbar = ({ isSticky, cartListRefetch }) => {
  ...
  const { countryCode, language, countryFlag } = useSelector(
  ...
  const customerbaseUrl = global?.base_urls?.customer_image_path;
  ...
  const handleAuthBasedOnRoute = () => {
    return (
      <RTL direction={languageDirection}>
        {!token ? (
          <Stack direction="row" paddingInline=".5rem" spacing={1}>
            <SellWithButton
              // onClick={handleOpenAuthModal}
              variant="contained"
            >
              <CustomStackFullWidth
                direction={languageDirection === "rtl" ? "row" : "row-reverse"}
                alignItems="center"
                spacing={1}
              >
                <CustomTypography
            ...
            <SignUpButton onClick={handleOpenAuthModal} variant="contained">
              <CustomStackFullWidth
                direction={languageDirection === "rtl" ? "row" : "row-reverse"}
                alignItems="center"
                spacing={1}
            ...
            <SignInButton onClick={handleOpenAuthModal} variant="contained">
              <CustomStackFullWidth
                direction={languageDirection === "rtl" ? "row" : "row-reverse"}
                alignItems="center"
                spacing={1}
  ...
  const handleShowSearch = () => {
    if ((router.pathname === "/home" && location) || openSearchBox) {
      ...
      return (
        ...
        >
          <CustomNavSearchIcon>
            <SearchOutlinedIcon sx={{ fontSize: "20px" }} color="primary" />
          </CustomNavSearchIcon>
        </Stack>
      );
    }
