    ...
    const onSuccessHandler = async (response) => {
      if (response.status === 203) {
        response?.data?.errors?.forEach(
          (item) => toast.error(item.message),
          console.log("Toasted from Loyality Modal ", item.message)
        );
      } else {
      ...
      >
        <BoxStyle>
          ...
          <CustomColouredTypography
            ...
            color="primary.main"
          >
            {`${global?.loyalty_point_exchange_rate} ${point} = ${getAmount(
              1,
          ...
          <Stack mt="1rem">
            <form noValidate onSubmit={loyalityFormik.handleSubmit}>
              <TextField
                ...
                helperText={
                  loyalityFormik.touched.point && loyalityFormik.errors.point
                }
              />
