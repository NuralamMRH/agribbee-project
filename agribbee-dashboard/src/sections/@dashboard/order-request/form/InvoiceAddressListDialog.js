...
InvoiceAddressListDialog.propTypes = {
  ...
  handleOpenNewAddressForm: PropTypes.func,
};
      ...
      {isNotFound ? (
        ...
        <Scrollbar sx={{ p: 1.5, pt: 0, maxHeight: 80 * 8 }}>
          {dataFiltered?.map((address) => (
            ...
            >
              <Typography variant="subtitle2">{address.address_type}</Typography>
              <Typography variant="subtitle2">{address.contact_person_name}</Typography>
...
function applyFilter(array, query) {
  if (query) {
    return array.filter(
      (address) =>
        address?.contact_person_name.toLowerCase().indexOf(query.toLowerCase()) !== -1 ||
        address?.zip.toLowerCase().indexOf(query.toLowerCase()) !== -1 ||
